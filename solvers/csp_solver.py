"""
CSP solver using weighted constraints and deterministic selection.
...
class CSPSolver:
    """CSP solver with weighted constraints and deterministic selection."""
    ...
    def _get_best_guess_deterministic(self, remaining: List[str]) -> List[str]:
        """
        Get best guess using deterministic selection (no random sampling).
        Scores all valid combinations and picks the best.
        ...
        # Generate all combinations (or a large deterministic subset)
        all_combos = list(combinations(remaining, 4))
        max_combos = min(500, len(all_combos))  # Check up to 500 combinations deterministically
        
        best_group = None
        best_score = float('-inf')
        
        # Sort deterministically (by word order) and check top N
        sorted_combos = sorted(all_combos)[:max_combos]
